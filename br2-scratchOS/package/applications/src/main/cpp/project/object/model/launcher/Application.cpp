...
#include <project/object/model/launcher/ApplicationPrivate.hpp>

using namespace project::object::model::launcher;

log4cxx::LoggerPtr Application::logger =
    log4cxx::Logger::getLogger(std::string("project.object.model.launcher.Application"));
...
PluginInterface & Application::getPlugin(const std::string& name)const
{
    ...
    if(!ptr)
    {
        LOG4CXX_ERROR(logger, "unable to find plugin: " << name );
    }
