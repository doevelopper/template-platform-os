...
#ifndef PROJECT_OBJECT_MODEL_LAUNCHER_EXECUTIONCONTEXT_HPP
#define PROJECT_OBJECT_MODEL_LAUNCHER_EXECUTIONCONTEXT_HPP
...
namespace project::object::model::launcher
{
    ...
    struct Priority
    {
        static constexpr int LOWEST      = std::numeric_limits<std::uint8_t>::min();
        ...
        static constexpr int HIGHEST     = std::numeric_limits<std::uint8_t>::max();
    };

    class ExecutionContext : public boost::asio::execution_context
    {
    ...
    private:
        ...
        std::priority_queue<std::unique_ptr<QueuedHandleBase>
            ,  std::deque<std::unique_ptr<QueuedHandleBase> >
            ,  derefLess> m_handlers;
        std::size_t m_order = std::numeric_limits<std::size_t>::max();
    };
}
