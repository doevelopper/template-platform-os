...
#ifndef PROJECT_OBJECT_MODEL_LAUNCHER_APPLICATION_HPP
#define PROJECT_OBJECT_MODEL_LAUNCHER_APPLICATION_HPP

#include <filesystem>
#include <typeindex>
#include <boost/core/demangle.hpp>

#include <project/object/model/launcher/Channel.hpp>
#include <project/object/model/launcher/ExecutionContext.hpp>
#include <project/object/model/launcher/Method.hpp>
#include <project/object/model/launcher/Plugin.hpp>
...
namespace project::object::model::launcher
{
    ...
    public:
        ...
        const boost::program_options::variables_map& get_options() const;
    ...
    protected:
        ...
        // friend class Plugin;
        bool initialize(int argc, char** argv, std::vector<PluginInterface*> autostartPlugins);

        void plugin_initialized(PluginInterface& plug);
        void plugin_started(PluginInterface& plug);

    private:

        Application();

        void startSighupHandler( std::shared_ptr<boost::asio::signal_set> sighup_set );
        void programOptions();
        void saveConfig(const std::filesystem::path& cfg_file);
        void configFileContent(std::ostream& os);
        void waitForSignal(std::shared_ptr<boost::asio::signal_set> ss);
        void iOServiceSignalHandling(boost::asio::io_service& ios, bool startup);
