 ...
 */
#ifndef PROJECT_OBJECT_MODEL_UTILS_COMPILER_DETECTION_HPP
#define PROJECT_OBJECT_MODEL_UTILS_COMPILER_DETECTION_HPP

//<prolog>

#define POM_NULLPTR                     nullptr
#define POM_DECL_EQ_DEFAULT             noexcept = default
#define POM_DECL_EQ_DELETE              noexcept = delete
#define POM_DECL_CONSTEXPR              constexpr
#define POM_CONSTEXPR                   const
#define POM_DECL_OVERRIDE               override
#define POM_DECL_FINAL                  final
#define POM_DECL_NOEXCEPT               noexcept
#define POM_DECL_NOEXCEPT_EXPR(x)       noexcept(x)
#define POM_FUNC_INFO                   __PRETTY_FUNCTION__
#define POM_FOREVER                     for(;;)
#define POM_REQUIRED_RESULT             [[nodiscard]]
#define POM_REQUIRED_RESULT_MSG(MSG)    [[nodiscard(MSG)]]
#define POM_DEPRECATED                  [[deprecated]]
#define POM_DEPRECATED_MSG(MSG)         [[deprecated(MSG)]]
#define POM_MAYBE_UNUSED                [[maybe_unused]]

#define POM_FORWARD_DECLARE_CLASS(name)     class name;
#define POM_FORWARD_DECLARE_STRUCT(name)    struct name;

#if defined(__cplusplus)
  #if __has_cpp_attribute(clang::fallthrough)
    #define POM_FALLTHROUGH() [[clang::fallthrough]]
  #elif __has_cpp_attribute(gnu::fallthrough)
    #define POM_FALLTHROUGH() [[gnu::fallthrough]]
  #elif __has_cpp_attribute(fallthrough)
    #define POM_FALLTHROUGH() [[fallthrough]]
  #endif
#endif

#ifndef POM_FALLTHROUGH
  #if (defined(__GNUC__) && __GNUC__ >= 700)
    #define POM_FALLTHROUGH() __attribute__((fallthrough))
  #else
    #define POM_FALLTHROUGH() (void)0
  #endif
#endif

#ifdef __GNUC__
#  define POM_NEVER_INLINE __attribute__((noinline))
#  define POM_ALWAYS_INLINE inline __attribute__((always_inline))
#  define POM_NORETURN __attribute__((__noreturn__))
#else
#  define POM_NEVER_INLINE
#  define POM_ALWAYS_INLINE inline
#  define POM_NORETURN
#endif
...
#define Q_DEFAULT_COPY_ASSIGN(Class)                                                                                   \
    Class(const Class &) POM_DECL_EQ_DEFAULT;                                                                          \
    Class &operator=(Class &&) POM_DECL_EQ_DEFAULT;

#define Q_DEFAULT_MOVE(Class)                                                                                          \
    Class(Class &&) POM_DECL_EQ_DEFAULT;                                                                               \
    Class &operator=(Class &&) POM_DECL_EQ_DEFAULT;

#define Q_DEFAULT_COPY(Class)                                                                                          \
    Class(const Class &) POM_DECL_EQ_DEFAULT;                                                                           \
    Class &operator=(const Class &) POM_DECL_EQ_DEFAULT;

#define Q_DISABLE_COPY_ASSIGN(Class)                                                                                   \
    Class(const Class &) POM_DECL_EQ_DELETE;                                                                           \
    Class &operator=(Class &&) POM_DECL_EQ_DELETE;

#define Q_DISABLE_COPY(Class)                                                                                          \
    Class(const Class &) POM_DECL_EQ_DELETE;                                                                           \
    Class &operator=(const Class &) POM_DECL_EQ_DELETE;

#define Q_DISABLE_MOVE(Class)                                                                                          \
    Class(Class &&) POM_DECL_EQ_DELETE;                                                                                \
    Class &operator=(Class &&) POM_DECL_EQ_DELETE;
