...
#ifndef PROJECT_OBJECT_MODEL_LOG_LOGGER_HPP
#define PROJECT_OBJECT_MODEL_LOG_LOGGER_HPP
 ...
 * @attention
 * <ul>
 *   <li>May change the scope of attributes to protected in not define in private section.</li>
 * </ul>
 * @retval None.
 */
 ...
 * @brief Define logger as private member.
 */
#define DECLARE_CLASS_LOGGGER(classname) \
...
#define LOG_DEBUG(logger,data) { \
        ...
        else LOG_CA(std::cout) << boost::posix_time::microsec_clock::local_time() << ": "  << data <<std::endl; \
}
...
#define LOG_ERROR(logger,data) { \
        ...
        else LOG_CA(std::cerr) << boost::posix_time::microsec_clock::local_time() << ": "  << data <<std::endl; \
}
...
#define DEBUG(x) \
    { time_t now = time(0); std::string s = std::string(ctime(&now)); \
...
namespace project::object::models::log
{
    ...
    public:
        ...
        POM_DEPRECATED_MSG("Replaced by LOG4CXX_TRACE macro, which has an improved interface")
        void trace ( const std::string & s );
        POM_DEPRECATED_MSG("Replaced by LOG4CXX_BEBUG macro, which has an improved interface")
        void debug ( const std::string & s );
        POM_DEPRECATED_MSG("Replaced by LOG4CXX_INFO macro, which has an improved interface")
        void info ( const std::string & s );
        POM_DEPRECATED_MSG("Replaced by LOG4CXX_WARN macro, which has an improved interface")
        void warn ( const std::string & s );
        POM_DEPRECATED_MSG("Replaced by LOG4CXX_ERROR macro, which has an improved interface")
        void error ( const std::string & s );
        POM_DEPRECATED_MSG("Replaced by LOG4CXX_FATAL macro, which has an improved interface")
    ...
    protected:
        ...
        // project::object::models::PimplPtr<LoggingServicePrivate> d_ptr;
